...
* **AREA (N)**: Your power affects a wide area instead of a single target. N is the amount of points spent on this Extra, and the increased area ranges.
...
* **Defensive**: You can use this typically offensive or utilitarian power to defend yourself or protect others.
* **Duration (N)**: Your power lasts longer than usual, based on its power level. N is the amount of points spent on this Extra, and the increased duration tiers.
* **INSTANT**: Your normally slow or Preparation-mandatory power now executes instantly.
* **PASSENGERS**: Your movement-related power can carry people or objects along.
* **RANGED (N)**: You can use a normally touch-based or Close-range power out to Near or Far range, OR you can increase the base Distance of this power N times (N is the amount of points spent on this Extra).
...
* **Sub-effect**: This power includes another power as a sub-effect of its own (that is usually only available if this power is active, or is a subproduct of it), i.e. Fire Aura might include Flight as a Sub-Effect.
...
* **Concentration**: You must concentrate to maintain the power, meaning that you can’t do other things properly in the meantime (DIFFICULTY for other tasks is increased), and certain hindrances could break that concentration, thus ending the effect.
* **constant**: The power is always active (making it hindering or very inconvenient). For damaging powers, this means you may unwillingly damage people and things around you. Other powers may make you appear freakish or just make life difficult for you.
* **Constrained to [x]**: Your power only works with a certain subset of its domain. For example, “Energy Control” usually covers all types of energy, but using this Limit you could constrain it to “Dark Energy Control”.
* **degrading**: Your power loses effectiveness over time when used. This can be represented as lowering its level on each Round that passes, or by a number indicated in the Power description on each use, until you have an opportunity to recover.
* **exclusive**: You cannot use or maintain other powers while using this one (either gets interrupted/turns off).
* **line-of-sight**: You must be able to see the target(s) of your power.
* **PREPARATION (N)**: You must spend N Speed Segments in a Round just winding-up the power before activating it. If N ends up longer than your Speed for that Round, it overflows to the next Round.
...
* **TIRING**: Using this power takes a toll; decrease one of your Attributes (the GM will tell you which) or this power by one tier, until rest.
